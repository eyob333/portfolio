
const proSvg = `<svg width="964" height="110" viewBox="0 0 964 110" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M56 55H923.5" stroke="white"/>
<circle cx="55" cy="55" r="45" fill="white"/>
<circle cx="694" cy="55" r="45" fill="white"/>
<circle cx="478" cy="55" r="45" fill="white"/>
<circle cx="257" cy="55" r="45" fill="white"/>
<circle cx="909" cy="55" r="45" fill="white"/>
<circle cx="909" cy="55" r="54.5" stroke="white"/>
<circle cx="55" cy="55" r="54.5" stroke="white"/>
</svg>
`;


const labSvg = `<svg width="962" height="97" viewBox="0 0 962 97" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M57 50H924.5" stroke="white"/>
<rect x="10" y="10" width="75" height="75" fill="#FFFEFE"/>
<rect x="875" y="10" width="80" height="80" fill="#FFFEFE"/>
<rect x="652" y="10" width="80" height="80" fill="#FFFEFE"/>
<rect x="439" y="10" width="80" height="80" fill="#FFFEFE"/>
<rect x="213" y="10" width="80" height="80" fill="#FFFEFE"/>
<rect x="0.5" y="0.5" width="94" height="94" stroke="white"/>
<rect x="867.5" y="2.5" width="94" height="94" stroke="white"/>
</svg>
`;
const modSvg = ` <svg width="972" height="90" viewBox="0 0 972 90" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M85.7369 79.5H18.1861L51.9615 20.999L85.7369 79.5Z" fill="#FFFCFC" stroke="white" stroke-width="11"/>
                                            <path d="M103.057 89.5H0.865822L51.9615 0.999023L103.057 89.5Z" stroke="white"/>
                                            <path d="M953.737 79.5H886.186L919.962 20.999L953.737 79.5Z" fill="#FFFCFC" stroke="white" stroke-width="11"/>
                                            <path d="M284.737 79.5H217.186L250.962 20.999L284.737 79.5Z" fill="#FFFCFC" stroke="white" stroke-width="11"/>
                                            <path d="M730.737 79.5H663.186L696.962 20.999L730.737 79.5Z" fill="#FFFCFC" stroke="white" stroke-width="11"/>
                                            <path d="M507.737 79.5H440.186L473.962 20.999L507.737 79.5Z" fill="#FFFCFC" stroke="white" stroke-width="11"/>
                                            <path d="M971.057 89.5H868.866L919.962 0.999023L971.057 89.5Z" stroke="white"/>
                                            <path d="M51.4615 54.5H918.962" stroke="#F2F2F2"/>
</svg>` ;  

export {modSvg, labSvg,proSvg}